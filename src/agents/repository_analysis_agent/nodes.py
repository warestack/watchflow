...
def _map_github_pr_to_signal(pr_data: dict[str, Any]) -> PRSignal:
    ...
    is_ai_generated = any(keyword in body or keyword in title for keyword in AI_DETECTION_KEYWORDS)
...
async def fetch_repository_metadata(state: AnalysisState) -> AnalysisState:
    ...
    try:
        ...
        if status_code is None and isinstance(e, aiohttp.ClientResponseError):
        ...
        if target in file_names:
            try:
                ...
                if content:
                    ...
                    break
            except (httpx.HTTPStatusError, aiohttp.ClientResponseError):
...
async def fetch_pr_signals(state: AnalysisState) -> AnalysisState:
    ...
    try:
        ...
        # Note: GraphQL requires auth. If agent is anonymous, this will fail or fallback.
        # Ideally, we should use installation_id if available, but for now we rely on potential env/fallback.
        pr_nodes = await github_client.fetch_pr_hygiene_stats(owner, repo_name)
        logger.info("debug_pr_nodes_fetched", count=len(pr_nodes) if pr_nodes else 0)
        ...
        if not pr_nodes:
            ...
            logger.info("debug_set_default_hygiene", summary=str(state.hygiene_summary))
            return state
        ...
        for pr in pr_nodes:
            ...
            if any(keyword in body or keyword in title for keyword in AI_DETECTION_KEYWORDS):
            ...
            pr_signals.append(
                PRSignal(
                    ...
                    is_ai_generated_hint=any(
                        ...
                        for keyword in AI_DETECTION_KEYWORDS
                    ),
        ...
        logger.error("debug_exception_fetch_pr_signals", error=str(e))
        logger.warning(
