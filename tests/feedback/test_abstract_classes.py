...
def verify_code_changes_direct():
    ...
    files_to_check = [
        ...
        ("agents/base.py", "BaseAgent", ["_build_graph", "execute"]),
        (
            "event_processors/base.py",
            "BaseEventProcessor",
            ["process", "get_event_type", "prepare_webhook_data", "prepare_api_data"],
        ),
        ("webhooks/handlers/base.py", "EventHandler", ["handle"]),
        ("rules/interface.py", "RuleLoader", ["get_rules"]),
        ("rules/validators.py", "Condition", ["validate"]),
    ]
        ...
        try:
            ...
            for i, line in enumerate(lines):
                ...
                if in_abstract_method:
                    ...
                    if (
                        re.match(r"^\s*(?:@|def\s+|class\s+|async\s+def\s+)", line)
                        and line_indent <= abstract_method_indent
                        and line_indent > 0
                    ):
                        in_abstract_method = False
                        continue
